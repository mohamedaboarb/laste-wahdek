  ...
  if (isLoading) {
    return (
      <div className="flex h-[60vh] w-full flex-col items-center justify-center gap-2 text-slate-500">
        ...
        <p className="text-sm font-medium">{p.loadingProfile}</p>
      </div>
    );
  }
  ...
  return (
    <FormProvider {...form}>
      ...
      >
        ...
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 text-slate-500">{p.pageSubtitle}</p>
            </div>
        ...
        <ChildrenSection isEditing={isEditing} onUploadingChange={setIsUploading} />

        {/* STICKY SAVE BAR
            On mobile: sits above the MobileBottomNav (h-16 = 4rem) using bottom-16.
            On desktop (lg+): sits at the very bottom (bottom-0). */}
        {isEditing && (
          <div className="fixed bottom-16 lg:bottom-0 left-0 right-0 z-[60] border-t bg-white/95 backdrop-blur">
            <div className="mx-auto flex max-w-7xl items-center justify-center md:justify-between px-6 py-4">
              <div className="hidden md:flex flex-col">
                <p className="font-medium text-slate-900">{p.unsavedChanges}</p>
                <p className="text-sm text-slate-500">{p.unsavedChangesHint}</p>
              </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  {isSubmitting ? p.saving : isUploading ? p.uploading : p.saveChanges}
                </button>
              </div>
            </div>
          </div>
